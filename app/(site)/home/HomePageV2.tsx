...
import { HSliderClient } from './HSliderClient'
import { LeadFormClient } from './LeadFormClient'
import { HeroBadgesClient } from './HeroBadgesClient'
...
const pricing = [
  { name: 'Сайт',                desc: 'Лендинг или корпоративный сайт', price: 'от 70 000 ₽', featured: false, features: ['Индивидуальный дизайн под бренд', 'Адаптив под все устройства', 'CMS для самостоятельного редактирования', 'Базовая SEO-настройка'] },
  ...
  { name: 'Поддержка сайтов',    desc: 'Сопровождение и доработка по часам', price: '2 500 ₽/час', featured: false, features: ['Исправление ошибок и доработки', 'Обновление CMS, модулей и зависимостей', 'Мониторинг доступности и резервные копии', 'Без абонплаты — оплата по затраченным часам', 'Реакция на срочные задачи в тот же день'] },
]
...
const faqData = [
  ...
  { q: 'Каковы типичные сроки?',                 a: 'Лендинг — 1–2 недели. Корпоративный сайт — 4–6 недель. Интернет-магазин — 6–10 недель. CRM или SaaS — от 3 месяцев. Сроки фиксируются в договоре и соблюдаются в 94% проектов.' },
...
function PriceCard({ p }: { p: typeof pricing[0] }) {
  return (
    <Box
      w="full"
      h="full"
    ...
    >
      ...
      <Text fontSize="15px" opacity={0.8} mb={5} color={p.featured ? 'white' : C.muted}>{p.desc}</Text>
      ...
      <Link href="#cta" style={{ display: 'block' }}>
        <Button
...
export function HomePageV2({ cmsServices, cmsPage }: HomePageV2Props) {
    ...
    <Box bg={C.bg} color={C.ink} overflowX="hidden" style={{ fontFamily: 'var(--font-manrope, Manrope, sans-serif)' }}>
      ...
      <Box as="section" minH="100vh" position="relative" overflow="hidden" display="flex" alignItems="center">
        ...
        <Container maxW="1000px" position="relative" zIndex={2} textAlign="center" py={20}>
          <Text mb={6} color={C.muted} fontSize="14px" letterSpacing="3px" textTransform="uppercase">
            {heroSupTitle}
          </Text>
          <Heading
            as="h1"
          ...
          <HStack mt={10} gap={6} justify="center" wrap="wrap">
            {heroMiniStats.map((s) => (
              <HStack key={s.label} gap={2}>
                ...
                <Text fontSize="sm" color={C.muted} fontWeight="600">{s.label}</Text>
              </HStack>
            ))}
          </HStack>
        </Container>
            ...
            {pageStats.map((s: typeof stats[0]) => (
              <Box key={s.label} bg="white" py={12} px={5} textAlign="center">
                <Text fontSize={{ base: '48px', md: '64px' }} fontWeight="900" lineHeight={1} style={gradText}>
                  <CountUpClient end={s.target} />{s.suffix}
                </Text>
                <Text mt={2} color={C.muted} fontSize="16px" fontWeight="500">{s.label}</Text>
              </Box>
            ))}
          </SimpleGrid>
        </Container>
              ...
              {displayServices.map((sv) => {
                const card = (
                  ...
                  >
                    ...
                    <Text color={C.muted} fontSize="15px" lineHeight="1.6" mb={6}>{sv.desc}</Text>
                    {sv.tags.length > 0 && (
                ...
                return (
                  ...
                  >
                    {sv.slug ? (
                      <Link href={`/uslugi/${sv.slug}`} style={{ display: 'flex', width: '100%', textDecoration: 'none', color: 'inherit' }}>
                        {card}
                      </Link>
                    ) : (
                      card
                ...
                <ScrollRevealClient key={i}>
                  ...
                  >
                    <Flex w="56px" h="56px" borderRadius="16px" align="center" justify="center" flexShrink={0} style={{ background: GRAD }}>
                      <Ic size={26} color="#ffffff" strokeWidth={2} />
                    </Flex>
                    <Box>
                      <Text fontSize="22px" fontWeight="800" mb={2}>{w.title}</Text>
                      <Text color={C.muted} fontSize="15px" lineHeight="1.6">{w.desc}</Text>
                    </Box>
                  </Flex>
                </ScrollRevealClient>
              )
            ...
            {pageFreelancer.map((p: typeof freelancerPointsDefault[0], i: number) => (
              <ScrollRevealClient key={i}>
                ...
                >
                  ...
                  <Text color={C.muted} fontSize="15px" lineHeight="1.6">{p.desc}</Text>
                </Box>
              </ScrollRevealClient>
            ))}
          </SimpleGrid>
        ...
        <Container maxW="1320px" px={{ base: 4, md: 6 }}>
          ...
          {pagePricing.length >= 4 ? (
            ...
            <SimpleGrid columns={{ base: 1, md: 3 }} gap={6} mt={12} alignItems="start">
              {pagePricing.map((p: typeof pricing[0], i: number) => (
                <ScrollRevealClient key={i}><PriceCard p={p} /></ScrollRevealClient>
              ))}
            </SimpleGrid>
          )}
        </Container>
