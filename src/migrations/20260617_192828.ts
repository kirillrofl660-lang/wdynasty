...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`tg_subscribers\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`tg_subscribers_chat_id_idx\` ON \`tg_subscribers\` (\`chat_id\`);`)
  await db.run(sql`CREATE INDEX \`tg_subscribers_updated_at_idx\` ON \`tg_subscribers\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`tg_subscribers_created_at_idx\` ON \`tg_subscribers\` (\`created_at\`);`)
  await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`leads_id\` integer REFERENCES leads(id);`)
  await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`tg_subscribers_id\` integer REFERENCES tg_subscribers(id);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`tg_subscribers\`;`)
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "users_id", "pages_id", "media_id", "projects_id", "skills_id", "posts_id", "navigation_id") SELECT "id", "order", "parent_id", "path", "users_id", "pages_id", "media_id", "projects_id", "skills_id", "posts_id", "navigation_id" FROM \`payload_locked_documents_rels\`;`)
